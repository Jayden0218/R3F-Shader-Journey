...
)
{
  ...
  // Want to standardize, make the bigger or smaller screen have the same gl_FragCoord
  vec2 uv = gl_FragCoord.xy / uResolution.y;
  // Make the gl_FragCoord bigger than modulu it, so that it wil make many small box of gl_FragCoord
  uv *= repetitions;
  uv = mod(uv,1.0);
  
  // Make it has a circle at each coord
